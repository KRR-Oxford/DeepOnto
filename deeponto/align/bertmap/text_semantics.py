...
class AnnotationThesaurus:
    ...
    def __init__(
        self, onto: Ontology, annotation_property_iris: List[str], apply_transitivity: bool = False
    ):
        ...
        self.average_number_of_annotations_per_class = total_number_of_annotations / len(
        ...
        self.info = {
            type(self).__name__: {
                ...
                "average_number_of_annotations_per_class": round(
                    self.average_number_of_annotations_per_class, 3
                ),
    ...
    def merge_synonym_groups_by_transitivity(synonym_groups: List[Set[str]]):
        ...
        for synonym_group in synonym_groups:
            ...
            synonym_pairs += AnnotationThesaurus.get_synonym_pairs(
                synonym_group, remove_duplicates=False
            )
    ...
    def soft_nonsynonym_sampling(self, num_samples: int, max_iter: int = 5):
        ...
        while len(nonsyonym_pool) < num_samples and max_iter > 0:
            ...
            nonsyonym_pool += self.soft_nonsynonym_sampling(
                num_samples - len(nonsyonym_pool), max_iter
            )
            nonsyonym_pool = uniqify(nonsyonym_pool)
    ...
    def hard_nonsynonym_sampling(self, num_samples: int, max_iter: int = 5):
        ...
        while len(nonsynonym_pool) < num_samples and max_iter > 0:
            max_iter = max_iter - 1  # reduce the iteration to prevent exhausting loop
            nonsynonym_pool += self.hard_nonsynonym_sampling(
                num_samples - len(nonsynonym_pool), max_iter
            )
            nonsynonym_pool = uniqify(nonsynonym_pool)
...
class IntraOntologyTextSemanticsCorpus:
    ...
    def __init__(
        ...
        self.thesaurus = AnnotationThesaurus(
            onto, annotation_property_iris, apply_transitivity=False
        )
        ...
        num_soft = (soft_negative_ratio + hard_negative_ratio) * len(self.synonyms) - len(
            self.hard_nonsynonyms
        )
...
class CrossOntologyTextSemanticsCorpus:
    ...
    def nonsynonym_sampling_from_mappings(self, num_samples: int, max_iter: int = 5):
        ...
        for class_mapping in self.class_mappings:
            ...
            cross_onto_synonym_group_pair.append(
                (src_class_annotations, tgt_class_annotations)
            )

        # randomly select disjoint synonym group pairs from all
        for _ in range(num_samples):
            left_class_pair, right_class_pair = tuple(
                random.sample(cross_onto_synonym_group_pair, 2)
            )
            # randomly choose one label from a synonym group
        ...
        while len(nonsynonym_pool) < num_samples and max_iter > 0:
            max_iter = max_iter - 1  # reduce the iteration to prevent exhausting loop
            nonsynonym_pool += self.nonsynonym_sampling_from_mappings(
                num_samples - len(nonsynonym_pool), max_iter
            )
            nonsynonym_pool = uniqify(nonsynonym_pool)
        return nonsynonym_pool
...
class TextSemanticsCorpora:
    ...
    def __init__(
        ...
        if self.class_mappings:
            self.cross_onto_corpus = CrossOntologyTextSemanticsCorpus(class_mappings, src_onto, tgt_onto, annotation_property_iris)
        ...
        if self.auxiliary_ontos:
            for auxiliary_onto in self.auxiliary_ontos:
                self.auxiliary_onto_corpora.append(IntraOntologyTextSemanticsCorpus(auxiliary_onto, annotation_property_iris))

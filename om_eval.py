...
@click.option("-a", "--ref_anchored_path", type=click.Path(exists=True), default=None)  # for local_rank
...
def onto_match_eval(
    ...
    ref_anchored_path: Optional[str],
    threshold: float,
    hits_at: List[int],
    ...
    if mode == "global_match":
        ...
        results = local_rank_eval(pred_path, ref_anchored_path, *hits_at)
    else:
